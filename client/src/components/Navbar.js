...
function Navbar() {
    ...
    const menuObjs = [ { id: 1, path: "/", name: "Create Cluster" }, { id: 2, path: "/dashboard", name: "Dashboard" }, { id: 3, path: "/nodes", name: "Nodes" }, { id: 4, path: "/pods", name: "Pods" }, { id: 5, path: "/services", name: "Services" }]
                    ...
                    <div className="flex items-center justify-between h-16">
                        <div className="flex items-center">
                            <div className="flex-shrink-0">
                                <h1 className="text-white text-[1.5rem]">KOS</h1>
                            </div>
                            {menuObjs.map((menu) => {
                                return (
                                    <div className="hidden md:block" key={menu.id}>
                                        <div className="ml-10 flex items-baseline space-x-2">
                                            <NavLink to={menu.path} className="text-white block px-1 py-2 rounded-md text-base font-medium">{menu.name}</NavLink>
                                        </div>
                                    </div>
                                )
                            })}
                        ...
                        <div className="hidden md:block">
                            <div className="ml-4 flex items-center md:ml-6">
                                ...
                                <div className="ml-3 relative">
                                    ...
                                    {openProfile === true && <div
                                        className="origin-top-right absolute right-0 mt-2 w-48 rounded-md shadow-lg py-1 bg-white ring-1 ring-black ring-opacity-5 focus:outline-none"
                                        role="menu" aria-orientation="vertical" aria-labelledby="user-menu-button"
                                        tabIndex="-1">

                                        <a href="#1" className="block px-4 py-2 text-sm text-gray-700"
